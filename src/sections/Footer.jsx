    ...
    <footer className="max-container">
      <div className=" flex justify-between items-start gap-20 flex-wrap max-lg:flex-col">
        ...
        <div className=" flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap">
            {footerLinks.map((footerSection) => (
              <div key={footerSection}>
                <h4 className=" text-white font-montserrat text-2xl leading-normal font-medium mb-6">{footerSection.title}</h4>
                <ul>{footerSection.links.map((link)=>(
                  <li key={link.name} className="mt-3 text-white-400 text-base leading-normal font-montserrat hover:text-slate-gray cursor-pointer"><a>{link.name}</a></li>
                ))}</ul>
              </div>
            ))}
        </div>
      ...
      <div className=" flex justify-between text-white-400 mt-24 max-sm:flex-col max-sm:items-center">
        <div className="flex flex-1 justify-start items-center gap-2 font-montserrat cursor-pointer">
          <img src={copyrightSign} alt="copyrighht" width={20} height={20} className=" rounded-full m-0"/>
          <p>Copyrighht. All rights reserved.</p>
        </div>
